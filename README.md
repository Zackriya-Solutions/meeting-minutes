...
| 5 | **Automatic call detection & recording** — Meetily watches for active calls (MS Teams on macOS via system audio, Windows/Linux via process detection) and automatically starts recording when a call begins. A native OS notification plus an in-app popup alert you when detection fires; a countdown prompt appears when the call ends so you can stop or continue recording. Enabled by default; toggled via Settings → Recordings → **Auto detect calls**. | ✅ Available |
