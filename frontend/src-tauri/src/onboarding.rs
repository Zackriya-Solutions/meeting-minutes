...
pub async fn complete_onboarding<R: Runtime>(
    ...
    info!(
        "Completing onboarding with defaults (OpenRouter Sonnet summary + SaluteSpeech transcription); local model choice recorded: {}",
        model
    );
    ...
    // Network LLM tasks use the configured OpenRouter key and the rolling Sonnet alias.
    ...
    .await
    {
        error!("Failed to save OpenRouter model config: {}", e);
        return Err(format!("Failed to save OpenRouter model config: {}", e));
    }
    info!(
        "Saved OpenRouter summary model config: model={}",
        crate::llm::providers::openrouter::DEFAULT_MODEL
    );

    // Transcription defaults to cloud SaluteSpeech (managed gateway, no local model).
    if let Err(e) =
        SettingsRepository::save_transcript_config(pool, "salutespeech", "salutespeech-stream-v2")
            .await
    {
