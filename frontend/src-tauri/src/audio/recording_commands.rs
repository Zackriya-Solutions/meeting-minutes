...
pub async fn start_recording_with_meeting_name<R: Runtime>(
    ...
    let transcript_config =
        crate::api::api::api_get_transcript_config(app.clone(), app.clone().state(), None)
            ...
            .ok()
            .flatten();
    let use_hosted_batch = transcript_config
        .as_ref()
        .map(|config| transcription::is_hosted_transcription_provider(&config.provider))
        .unwrap_or(false);

    // Hosted providers run after recording is saved. Drain live chunks so the audio
    // pipeline does not accumulate unbounded speech buffers during long meetings.
    let task_handle = if use_hosted_batch {
        info!("☁️ Hosted batch transcription selected; draining live transcription chunks");
        tokio::spawn(async move {
            let mut receiver = transcription_receiver;
            while receiver.recv().await.is_some() {}
            info!("☁️ Hosted transcription drain task completed");
        })
    } else {
