                ...
                Ok(Some(chunk)) => {
                    ...
                    while self.ring_buffer.can_mix() {
                        if let Some((mic_window, sys_window)) = self.ring_buffer.extract_window() {
                            ...
                            let mic_rms: f32 = if mic_window.is_empty() {
                                0.0
                            } else {
                                (mic_window.iter().map(|&s| s * s).sum::<f32>()
                                    / mic_window.len() as f32)
                                    .sqrt()
                            };
                            let sys_rms: f32 = if sys_window.is_empty() {
                                0.0
                            } else {
                                (sys_window.iter().map(|&s| s * s).sum::<f32>()
                                    / sys_window.len() as f32)
                                    .sqrt()
                            };
