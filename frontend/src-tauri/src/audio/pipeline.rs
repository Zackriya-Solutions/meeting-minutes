...
impl AudioPipeline {
    ...
    fn send_speech_segment(&mut self, segment: SpeechSegment, is_final: bool) {
        ...
        for piece in
            crate::audio::common::split_segment_at_silence(&segment, MAX_ASR_SEGMENT_SAMPLES)
        {
            let duration_ms = piece.end_timestamp_ms - piece.start_timestamp_ms;
            if piece.samples.len() < MIN_ASR_SEGMENT_SAMPLES {
                debug!(
                    "⏭️ Dropping short VAD segment: {:.1}ms ({} samples < {})",
                    duration_ms,
                    piece.samples.len(),
                    MIN_ASR_SEGMENT_SAMPLES
                );
                continue;
            }
...
mod tests {
    ...
    fn tracker_prunes_old_windows() {
        ...
        assert!(
            tracker.windows.len() <= (ChannelEnergyTracker::MAX_RETAIN_MS / 500.0) as usize + 1
        );
    }
