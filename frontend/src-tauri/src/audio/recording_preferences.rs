...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\pulse-talq-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("pulse-talq-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("pulse-talq-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/pulse-talq-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("pulse-talq-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/pulse-talq-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("pulse-talq-recordings")
    }
