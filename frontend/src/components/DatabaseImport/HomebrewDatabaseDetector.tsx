...
export function HomebrewDatabaseDetector({ onImportSuccess, onDecline }: HomebrewDatabaseDetectorProps) {
  ...
  return (
    <div className="mb-4 p-4 bg-[var(--gold-soft)] border-2 border-[var(--gold-border)] rounded-lg">
      <div className="flex items-start gap-3">
        <Database className="h-6 w-6 text-[var(--gold)] mt-0.5 flex-shrink-0" />
        <div className="flex-1">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-[var(--success)] text-[var(--fg-inverse)] rounded-lg hover:bg-[var(--success)] disabled:bg-[var(--fg3)] disabled:cursor-not-allowed transition-colors"
            >
              {isImporting ? (
                <>
