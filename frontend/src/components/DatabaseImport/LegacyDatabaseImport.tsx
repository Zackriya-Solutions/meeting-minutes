...
export function LegacyDatabaseImport({ isOpen, onComplete }: LegacyDatabaseImportProps) {
  ...
  const handleBrowse = async () => {
    try {
      ...
      if (dbPath) {
        ...
        setErrorMessage(t('databaseImport.databaseNotFound'));
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      toast.success(t('databaseImport.importSuccessReloading'));
      ...
      setErrorMessage(String(error));
      setImportState('error');
      toast.error(t('databaseImport.importFailedWithError', { error: String(error) }));
      setTimeout(() => setImportState('idle'), 3000);
    }
  };
  ...
  const handleStartFresh = async () => {
    try {
      ...
      setImportState('success');
      toast.success(t('databaseImport.freshDatabaseReady'));
      ...
      setErrorMessage(String(error));
      setImportState('error');
      toast.error(t('databaseImport.initializationFailed', { error: String(error) }));
      setTimeout(() => setImportState('idle'), 3000);
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-[600px]" onPointerDownOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle className="text-2xl">{t('onboarding.welcome')}</DialogTitle>
          <DialogDescription className="text-base pt-2">
            {t('onboarding.previousDatabaseSubtitle')}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-3">
            <p className="text-sm text-gray-600">
              {t('databaseImport.selectPreviousData')}
            </p>
            ...
            >
              {importState === 'selecting' || importState === 'detecting' ? (
                <>
                  ...
                  <span>{importState === 'selecting' ? t('databaseImport.selecting') : t('databaseImport.detecting')}</span>
                </>
              ) : (
                <>
                  <FolderOpen className="h-5 w-5" />
                  <span>{t('onboarding.browseForDatabase')}</span>
                </>
              )}
            </button>
          </div>
            ...
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-green-800">{t('onboarding.databaseFound')}</p>
          ...
          <div className="flex flex-col gap-3 pt-2">
            ...
            >
              {importState === 'importing' ? (
                <>
                  <Loader2 className="h-5 w-5 animate-spin" />
                  <span>{t('onboarding.importing')}</span>
                </>
              ) : importState === 'success' ? (
                <>
                ...
                <>
                  ...
                  <span>{t('databaseImport.importDatabase')}</span>
                </>
              )}
            </button>
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-white text-gray-500">{t('databaseImport.or')}</span>
              </div>
            </div>
