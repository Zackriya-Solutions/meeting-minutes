...
export function ModelSettingsModal({
  ...
  const fetchOllamaModels = async (silent = false) => {
    ...
    if (trimmedEndpoint && !validateOllamaEndpoint(trimmedEndpoint)) {
      const errorMsg = t('modelSettings.invalidOllamaEndpoint');
      setError(errorMsg);
      if (!silent) {
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : t('modelSettings.failedLoadOllamaModels');
      setError(errorMsg);
  ...
  const loadOpenRouterModels = async () => {
    ...
    try {
      ...
      setOpenRouterError(
        err instanceof Error ? err.message : t('modelSettings.failedLoadOpenRouterModels')
      );
    } finally {
  ...
  const loadBuiltinAiModels = async () => {
    ...
    try {
      ...
      toast.error(t('modelSettings.failedLoadBuiltInModels'));
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (modelConfig.provider === 'custom-openai') {
      try {
        ...
        toast.error(t('modelSettings.failedLoadCustomOpenAI'));
        return;
      }
    }
  ...
  const testCustomOpenAIConnection = async () => {
    if (!customOpenAIEndpoint.trim() || !customOpenAIModel.trim()) {
      toast.error(t('modelSettings.pleaseEnterEndpointAndModel'));
      return;
    }
  ...
  const downloadRecommendedModel = async () => {
    ...
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : t('modelSettings.modelDownloadFailed', { model: recommendedModel });
      console.error('Error downloading model:', err);

      // Check if Ollama is not installed and show appropriate error
      if (isOllamaNotInstalledError(errorMsg)) {
        toast.error(t('modelSettings.ollamaNotInstalled'), {
          description: t('modelSettings.ollamaInstallDescription'),
          duration: 7000,
          action: {
            label: t('modelSettings.downloadAction'),
  ...
  const deleteOllamaModel = async (modelName: string) => {
    try {
      ...
      const errorMsg = err instanceof Error ? err.message : t('modelSettings.modelDeleteFailed', { model: modelName });
      toast.error(errorMsg);
      ...
      <div className="space-y-4">
          ...
          <div className="flex space-x-2 mt-1">
            ...
            >
              ...
              <SelectContent className="max-h-64 overflow-y-auto">
                ...
                <SelectItem value="claude">{t('modelSettings.providerClaude')}</SelectItem>
                <SelectItem value="custom-openai">{t('modelSettings.providerCustomOpenAI')}</SelectItem>
                <SelectItem value="groq">{t('modelSettings.providerGroq')}</SelectItem>
                <SelectItem value="ollama">{t('modelSettings.providerOllama')}</SelectItem>
                <SelectItem value="openai">{t('modelSettings.providerOpenAI')}</SelectItem>
                <SelectItem value="openrouter">{t('modelSettings.providerOpenRouter')}</SelectItem>
              </SelectContent>
            </Select>
        ...
        {modelConfig.provider === 'custom-openai' && (
          <div className="space-y-4 border-t pt-4">
            <div>
              <Label htmlFor="custom-endpoint">{t('modelSettings.endpointUrlLabel')}</Label>
              <Input
                id="custom-endpoint"
                value={customOpenAIEndpoint}
                onChange={(e) => setCustomOpenAIEndpoint(e.target.value)}
                placeholder={t('modelSettings.endpointUrlPlaceholder')}
                className="mt-1"
              />
              <p className="text-xs text-muted-foreground mt-1">
                {t('modelSettings.endpointUrlHelp')}
              </p>
            </div>

            <div>
              <Label htmlFor="custom-model">{t('modelSettings.modelNameLabel')}</Label>
              <Input
                id="custom-model"
              ...
              />
              <p className="text-xs text-muted-foreground mt-1">
                {t('modelSettings.modelNameHelp')}
              </p>
            </div>

            <div>
              <Label htmlFor="custom-api-key">{t('modelSettings.apiKeyLabel')}</Label>
              <Input
                id="custom-api-key"
                type="password"
        ...
        {requiresApiKey && (
          <div>
            <Label>{t('modelSettings.apiKeyLabel')}</Label>
        ...
        {modelConfig.provider === 'ollama' && (
          <div>
            ...
            >
              <Label className="cursor-pointer">{t('modelSettings.customEndpointTitle')}</Label>
            ...
            {!isEndpointSectionCollapsed && (
              <>
                <p className="text-sm text-muted-foreground mt-1 mb-2">
                  {t('modelSettings.customEndpointDescription')}
                </p>
                <div className="flex gap-2 mt-1">
                  <div className="relative flex-1">
                    <Input
                      ...
                      placeholder={t('modelSettings.ollamaEndpointPlaceholder')}
                      className={cn(
                        "pr-10",
                ...
                {ollamaEndpointChanged && !error && (
                  <Alert className="mt-3 border-yellow-500 bg-yellow-50">
                    <AlertDescription className="text-yellow-800">
                      {t('modelSettings.endpointChanged')}
                    </AlertDescription>
                  </Alert>
                )}
              </>
          ...
          <div>
            ...
            {isLoadingOllama ? (
              <div className="text-center py-8 text-muted-foreground">
                ...
                {t('modelSettings.loadingModelsMessage')}
              </div>
            ) : models.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    <Alert className="border-orange-500 bg-orange-50">
                      <AlertDescription className="text-orange-800">
                        {t('modelSettings.downloadOllamaDescription')}
                      </AlertDescription>
                    </Alert>
                    <Button
                      variant="default"
                    ...
                    <Alert className="mb-4">
                      <AlertDescription>
                        {ollamaEndpointChanged
                          ? t('modelSettings.endpointChanged')
                          : t('modelSettings.noOllamaModelsFound')}
                      </AlertDescription>
                    </Alert>
                    {!ollamaEndpointChanged && (
                      <div className="space-y-3">
                        ...
                        {isDownloading('gemma3:1b') && getProgress('gemma3:1b') !== undefined && (
                          <div className="bg-white rounded-md border p-3">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-sm font-medium text-blue-600">{t('modelSettings.downloadingRecommended')}</span>
              ...
              <ScrollArea className="max-h-[calc(100vh-450px)] overflow-y-auto pr-4">
                {filteredModels.length === 0 ? (
                  <Alert>
                    <AlertDescription>
                      {t('modelSettings.noModelsMatching', { query: searchQuery })}
                    </AlertDescription>
                  </Alert>
                ) : (
                      ...
                      return (
                        ...
                        >
                          <div>
                            ...
                            <span className="text-muted-foreground">{t('modelSettings.modelSize')} </span>
                          ...
                          {modelIsDownloading && progress !== undefined && (
                            <div className="mt-3 pt-3 border-t border-gray-200">
                              <div className="flex items-center justify-between mb-2">
                                <span className="text-sm font-medium text-blue-600">{t('modelSettings.modelDownloadProgress')}</span>
