...
export function DeviceSelection({ selectedDevices, onDeviceChange, disabled = false }: DeviceSelectionProps) {
  ...
  const fetchDevices = async () => {
    try {
      ...
      setError(t('deviceSelection.failedLoad'));
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const startAudioLevelMonitoring = async () => {
    try {
      ...
      if (deviceNames.length === 0) {
        setError(t('deviceSelection.noMicrophonesFound'));
        return;
      }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-900">{t('deviceSelection.title')}</h4>
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <Label htmlFor="mic-selection" className="text-sm font-medium text-gray-700">
              {t('deviceSelection.microphoneLabel')}
            </Label>
          </div>
          <Select
          ...
          >
            <SelectTrigger id="mic-selection" className="w-full">
              <SelectValue placeholder={t('deviceSelection.selectMicrophone')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="default">{t('deviceSelection.defaultMicrophone')}</SelectItem>
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('deviceSelection.noMicrophonesFound')}</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-gray-100">
              <p className="text-xs text-gray-600 font-medium">{t('deviceSelection.microphoneLevels')}</p>
              {inputDevices.map((device) => {
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <Label htmlFor="system-selection" className="text-sm font-medium text-gray-700">
              {t('deviceSelection.systemAudioLabel')}
            </Label>
          </div>
          ...
          >
            <SelectTrigger id="system-selection" className="w-full">
              <SelectValue placeholder={t('deviceSelection.selectSystemAudio')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="default">{t('deviceSelection.defaultSystemAudio')}</SelectItem>
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('deviceSelection.noSystemAudioFound')}</p>
          )}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• <strong>{t('deviceSelection.microphone')}</strong>: {t('deviceSelection.microphoneInfo')}</p>
        <p>• <strong>{t('deviceSelection.systemAudio')}</strong>: {t('deviceSelection.systemAudioInfo')}</p>
        {isMonitoring && (
          <p>• <strong>{t('deviceSelection.microphoneLevels')}</strong> {t('deviceSelection.levelsLegend')}</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• <strong>{t('deviceSelection.tip')}</strong> {t('deviceSelection.testMic')}</p>
        )}
      </div>
    </div>
  );
