...
export const RecordingControls: React.FC<RecordingControlsProps> = ({
  ...
  useEffect(() => {
    const checkTauri = async () => {
      try {
        ...
        alert(t('recording.errors.initFailed'));
      }
    };
    checkTauri();
  }, [t]);
  ...
  const handleStartRecording = useCallback(async () => {
    ...
    try {
      ...
      if (errorMsg.includes('microphone') || errorMsg.includes('mic') || errorMsg.includes('input')) {
        setDeviceError({
          title: t('recording.errors.micNotAvailable'),
          message: t('recording.errors.micNotAvailableMessage')
        });
      } else if (errorMsg.includes('system audio') || errorMsg.includes('speaker') || errorMsg.includes('output')) {
        setDeviceError({
          title: t('recording.errors.systemAudioNotAvailable'),
          message: t('recording.errors.systemAudioNotAvailableMessage')
        });
      } else if (errorMsg.includes('permission')) {
        setDeviceError({
          title: t('recording.errors.permissionRequired'),
          message: t('recording.errors.permissionRequiredMessage')
        });
      } else {
        setDeviceError({
          title: t('recording.errors.recordingFailed'),
          message: t('recording.errors.recordingFailedMessage')
        });
      }
    } finally {
      setIsStarting(false);
  ...
  }, [onRecordingStart, isStarting, selectedDevices, meetingName, t]);
  ...
  const handlePauseRecording = useCallback(async () => {
    ...
    try {
      ...
      alert(t('recording.errors.pauseFailed'));
    } finally {
      setIsPausing(false);
    }
  ...
  const handleResumeRecording = useCallback(async () => {
    ...
    try {
      ...
      alert(t('recording.errors.resumeFailed'));
    } finally {
      setIsResuming(false);
    }
                ...
                <>
                  {!isRecording ? (
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>
                        <p>{t('recording.tooltips.start')}</p>
                      </TooltipContent>
                    </Tooltip>
                  ) : (
                    ...
                    <>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p>{isPaused ? t('recording.tooltips.resume') : t('recording.tooltips.pause')}</p>
                        </TooltipContent>
                      </Tooltip>
