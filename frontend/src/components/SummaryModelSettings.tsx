...
export function SummaryModelSettings({ refetchTrigger }: SummaryModelSettingsProps) {
  ...
  const fetchModelConfig = useCallback(async () => {
    try {
      ...
      toast.error(t('modelSettings.failedLoadSettings'));
    }
  }, []);
  ...
  const handleSaveModelConfig = async (config: ModelConfig) => {
    try {
      ...
      toast.success(t('modelSettings.savedSettings'));
    } catch (error) {
      console.error('Error saving model config:', error);
      toast.error(t('modelSettings.failedSaveSettings'));
    }
  };
    ...
    <div className='flex flex-col gap-4'>
      <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 mb-2">{t('summary.autoSummaryTitle')}</h3>
            <p className="text-sm text-gray-600">{t('summary.autoSummaryDescription')}</p>
          </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm">
        <h3 className="text-lg font-semibold mb-4">{t('summary.modelConfigurationTitle')}</h3>
        <p className="text-sm text-gray-600 mb-6">
          {t('summary.modelConfigurationDescription')}
        </p>
