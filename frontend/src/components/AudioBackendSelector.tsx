  ...
  if (loading) {
    return (
      <div className="animate-pulse">
        <div className="h-4 bg-ink/10 rounded w-32 mb-2"></div>
    ...
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        <div className="relative">
          ...
          {showTooltip && (
            <div className="absolute z-10 left-6 top-0 w-64 p-3 text-xs bg-ink text-canvas rounded-lg shadow-lg">
      ...
      {error && (
        <div className="p-2 text-xs text-danger-ink bg-danger-soft border border-danger/40 rounded-md">
          {error}
        </div>
      )}
          ...
          return (
            ...
            >
              <input
                ...
                disabled={isDisabled}
                className="mt-1 h-4 w-4 text-info-ink focus:ring-ring border-line"
              />
              <div className="ml-3 flex-1">
                <div className="flex items-center justify-between">
                  ...
                  {currentBackend === backend.id && (
                    <span className="text-xs font-medium text-info-ink bg-info-soft px-2 py-0.5 rounded">
                      Active
                    </span>
                  )}
                  {isCoreAudio && (
                    <span className="text-xs font-medium text-ink-muted bg-sunken px-2 py-0.5 rounded">
                      Disabled
                    </span>
                  )}
                </div>
