  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm relative">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="text-lg font-semibold text-gray-900">{t('summary.summaryLanguage')}</h3>
      </div>
      <p className="text-sm text-gray-600 mb-4">
        {t('summary.languageSettingsDescription')}
      </p>
        ...
        {recents.map((code) => {
          ...
          return (
            ...
            >
              <button
                type="button"
                aria-label={isPinned ? t('summary.unpinLanguage', { language: labelForCode(code) }) : t('summary.pinLanguage', { language: labelForCode(code) })}
              ...
              <button
                type="button"
                aria-label={t('summary.removeLanguage', { language: labelForCode(code) })}
      ...
      <p className="text-xs text-gray-400 mt-3">
        {pinned
          ? t('summary.defaultLanguage', { language: labelForCode(pinned) })
          : t('summary.selectDefaultLanguage')}
      </p>
    </div>
  );
}
