...
export function SummaryGeneratorButtonGroup({
  ...
  const checkBuiltInAIModelsAndGenerate = async () => {
    ...
    try {
      ...
      if (!selectedModel) {
        toast.error(t('summary.noBuiltInModelSelected'), {
          description: t('summary.selectModelInSettings'),
          duration: 5000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      if (!modelInfo) {
        toast.error(t('summary.modelNotFound'), {
          description: `${t('summary.modelNotFound')}: ${selectedModel}`,
          duration: 5000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      if (status.type === 'downloading') {
        toast.info(t('summary.modelDownloadInProgress'), {
              description: `${selectedModel} (${status.progress}%)`,
          duration: 5000,
        });
        return;
      }
      ...
      if (status.type === 'not_downloaded') {
        toast.error(t('summary.modelNotDownloaded'), {
              description: `${selectedModel}: ${t('summary.modelNotDownloaded')}`,
          duration: 5000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      if (status.type === 'corrupted') {
        toast.error(t('summary.modelError'), {
              description: `${selectedModel}: ${t('modelSettings.modelErrorDescription')}`,
          duration: 7000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      if (status.type === 'error') {
        toast.error(t('summary.modelError'), {
              description: status.Error || t('modelSettings.modelGenericError'),
          duration: 5000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      // Fallback
      toast.error(t('summary.modelNotAvailable'), {
        description: t('summary.modelNotReady'),
        duration: 5000,
      });
      setSettingsDialogOpen(true);
      ...
      toast.error(t('summary.failedCheckModelStatus'), {
  ...
  const checkOllamaModelsAndGenerate = async () => {
    ...
    try {
      ...
      const models = await invoke<unknown[]>('get_ollama_models', { endpoint });
      ...
      if (isOllamaNotInstalledError(errorMessage)) {
        ...
        toast.error(
          ...
          {
            description: t('summary.ollamaInstallDescription'),
            duration: 7000,
            action: {
    ...
    <ButtonGroup>
      ...
      {isGenerating ? (
        <Button
          ...
          title={t('summary.stopSummaryGeneration')}
        >
        ...
        <Button
          ...
          title={
            isModelConfigLoading
              ...
              : isCheckingModels
                ? t('common.searching')
                : hasSummary ? t('summary.generateSummary') : t('summary.generateSummary')
          }
        >
          {isCheckingModels || isModelConfigLoading ? (
            <>
              <Loader2 className="animate-spin xl:mr-2" size={18} />
              <span className="hidden xl:inline">{t('common.loading')}</span>
            </>
          ) : (
            <>
              <Sparkles className="xl:mr-2" size={18} />
              <span className="hidden lg:inline xl:inline">{hasSummary ? t('summary.generateSummary') : t('summary.generateSummary')}</span>
            </>
          )}
        </Button>
      )}
      ...
      <Dialog open={settingsDialogOpen} onOpenChange={setSettingsDialogOpen}>
        <DialogTrigger asChild>
          <Button
            variant="outline"
            size="sm"
              title={t('summary.summarySettings')}
          >
            <Settings />
            <span className="hidden lg:inline">{t('modelSettings.title')}</span>
          </Button>
        </DialogTrigger>
        <DialogContent
      ...
      {availableTemplates.length > 0 && (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              <FileText />
              <span className="hidden lg:inline">{t('summary.generateSummary')}</span>
            </Button>
          </DropdownMenuTrigger>
