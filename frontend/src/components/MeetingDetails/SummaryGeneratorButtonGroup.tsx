...
export function SummaryGeneratorButtonGroup({
  ...
  const checkBuiltInAIModelsAndGenerate = async () => {
    ...
    try {
      ...
      if (!selectedModel) {
        toast.error(t('misc.noModelsDownload'), {
          description: t('transcriptSettings.modelSetupRequired'),
          duration: 5000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      if (status.type === 'downloading') {
        toast.info(t('onboarding.downloadingUpdate'), {
          description: `${t('modelSettings.builtInAIModels')} ${selectedModel} (${status.progress}%). ${t('onboarding.continueWhileDownloading')}`,
          duration: 5000,
        });
        return;
      }
      ...
      if (status.type === 'not_downloaded') {
        toast.error(t('transcriptSettings.modelFileError'), {
          description: `${selectedModel} ${t('transcriptSettings.modelSetupRequired')}`,
          duration: 5000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      if (status.type === 'corrupted') {
        toast.error(t('transcriptSettings.modelFileCorrupted'), {
          description: `${selectedModel} ${t('about.updateFailed', { message: '' })}`,
          duration: 7000,
        });
        setSettingsDialogOpen(true);
        return;
      ...
      if (isOllamaNotInstalledError(errorMessage)) {
        ...
        toast.error(
          t('modelSettings.ollamaNotRunning').split(':')[0].trim(),
          {
            ...
            duration: 7000,
            action: {
              label: t('onboarding.downloadAndInstall'),
        ...
        toast.error(
          t('modelSettings.connectionFailed'),
          { duration: 5000 }
        );
      }
      setSettingsDialogOpen(true);
    ...
    <ButtonGroup>
      ...
      {isGenerating ? (
        ...
        >
          ...
          <span className="hidden lg:inline xl:inline">{t("common.close")}</span>
        </Button>
      ) : (
        <Button
          variant="outline"
          ...
          title={
            isModelConfigLoading
              ...
              : isCheckingModels
                ...
                : hasSummary ? t('summarySettings.regenerate') : t('summarySettings.generateSummary')
          }
        >
          {isCheckingModels || isModelConfigLoading ? (
            <>
              <Loader2 className="animate-spin xl:mr-2" size={18} />
              <span className="hidden xl:inline">{t("statusOverlays.saving")}</span>
            </>
          ) : (
            <>
              <Sparkles className="xl:mr-2" size={18} />
              <span className="hidden lg:inline xl:inline">{hasSummary ? t('summarySettings.regenerate') : t('summarySettings.generateSummary')}</span>
            </>
          )}
        </Button>
      )}
      ...
      {availableTemplates.length > 0 && (
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            ...
            >
              <FileText />
              <span className="hidden lg:inline">{t("summarySettings.template")}</span>
            </Button>
          </DropdownMenuTrigger>
