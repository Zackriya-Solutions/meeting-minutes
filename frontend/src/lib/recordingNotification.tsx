 ...
 *
 * @param t - translate function from useI18n(), used to localize the toast copy
 * @returns Promise<void> - Resolves when notification is shown or skipped
 */
export async function showRecordingNotification(t: (key: string) => string): Promise<void> {
  try {
    ...
    if (showNotification) {
      ...
      const toastId = toast.info(t('recording.notificationTitle'), {
        description: (
          <div className="space-y-3 min-w-[280px]">
            <p className="text-sm font-medium text-gray-900">
              {t('recording.notificationBody')}
            </p>
            <label className="flex items-center gap-2 text-xs cursor-pointer hover:bg-blue-100 p-2 rounded transition-colors">
              ...
              />
              <span className="select-none text-gray-700">{t('recording.notificationDontShowAgain')}</span>
            </label>
            <button
              onClick={async () => {
                if (dontShowAgain) {
            ...
            >
              {t('recording.notificationAcknowledge')}
            </button>
          </div>
        ),
        duration: 10000,
