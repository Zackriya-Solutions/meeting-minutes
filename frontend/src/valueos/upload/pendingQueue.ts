...
export interface FlushOutcome {
  ...
  retained: string[]; // ids kept for a later retry
...
export class PendingUploadQueue {
  ...
  /** Try to upload every pending item. Safe to call repeatedly. */
  async flush(): Promise<FlushOutcome> {
    const outcome: FlushOutcome = { uploaded: [], retained: [], needsReauth: false };
    for (const item of await this.store.list()) {
      try {
        ...
        const err = e as ValueOsApiError;
        if (err instanceof ValueOsApiError && err.isAuth) {
          // 401 → stop; re-auth is required before any further attempt. Keep the item.
          outcome.needsReauth = true;
          outcome.retained.push(item.id);
          break;
        }
        // network / 503 / other → retain with an incremented attempt count. Never dropped.
