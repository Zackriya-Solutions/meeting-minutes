...
Injects CSV-based transcript data into the Conversationaly SQLite database,
...
def get_default_db_path() -> Path:
    ...
    if system == "Darwin":  # macOS
        base_path = Path.home() / "Library" / "Application Support" / "Conversationaly"
    elif system == "Windows":
        appdata = os.environ.get("APPDATA", "")
        if appdata:
            base_path = Path(appdata) / "Conversationaly"
        else:
            base_path = Path.home() / "AppData" / "Roaming" / "Conversationaly"
    else:  # Linux and others
        base_path = Path.home() / ".config" / "Conversationaly"
...
def main():
    parser = argparse.ArgumentParser(
        description="Inject CSV transcript data into Conversationaly database",
...
Example usage:
  ...
  python inject_transcript.py --csv data.csv --db ~/custom/path.sqlite
    ...
    if not db_path.exists():
        ...
        print("Make sure Conversationaly has been run at least once to create the database.", file=sys.stderr)
        sys.exit(1)
    ...
    print("\nThe meeting should now appear in the Conversationaly sidebar.")
