...
## Local install on this host (Omarchy)

A working build of this app lives at `~/apps/meetily/` (AppImage + patched source at v0.4.0).
See `~/apps/meetily/README-install.md` for layout, local build patches (CPU build, whisper-rs bump),
and the recording workflow (`record-meeting.sh` → Import & Enhance). Do not modify the read-only
reference clone at `~/Projects/squad/projects/meetily`.
