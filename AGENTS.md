...
- Do not add, restore, or enable `.github/workflows/quality-gate.yml` or any replacement "Quality Gate" workflow.
- Do not introduce automatic full-suite CI gates for pull requests or pushes unless the repository owner explicitly requests one.
- Keep the Claude pull-request reviewer workflow separate from build and test automation; reviewer changes must not recreate the removed Quality Gate.
