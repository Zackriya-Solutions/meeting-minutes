...
class DatabaseManager:
    ...
    async def create_meeting(self, title: str, date: str, time: Optional[str] = None,
        ...
        async with self._get_connection() as conn:
            await conn.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (meeting_id, title, date, time, json.dumps(attendees or []),
                 json.dumps(tags or []), content or "", now, now)
            )
            await conn.commit()
    ...
    async def get_meetings(self) -> List[Dict[str, Any]]:
        ...
        async with self._get_connection() as conn:
            async with conn.execute(
                ...
                return [
                    {col: (json.loads(row[idx]) if col in ("attendees", "tags") and row[idx] else row[idx])
    ...
    async def get_meeting(self, meeting_id: str) -> Optional[Dict[str, Any]]:
        ...
        async with self._get_connection() as conn:
            async with conn.execute(
                ...
                if row:
                    columns = [col[0] for col in cursor.description]
                    return {col: (json.loads(row[idx]) if col in ("attendees", "tags") and row[idx] else row[idx])
                            for idx, col in enumerate(columns)}
                return None
    ...
    async def update_meeting(self, meeting: Dict[str, Any]) -> None:
        ...
        async with self._get_connection() as conn:
            await conn.execute(
                """
                UPDATE meetings SET title = ?, date = ?, time = ?, attendees = ?, tags = ?, 
                content = ?, updated_at = ? WHERE id = ? AND deleted_at IS NULL
                """,
                (meeting['title'], meeting['date'], meeting.get('time'),
                 json.dumps(meeting.get('attendees', [])), json.dumps(meeting.get('tags', [])),
                 meeting.get('content', ""), now, meeting['id'])
            )
            await conn.commit()
    ...
    async def delete_meeting(self, meeting_id: str) -> None:
        ...
        async with self._get_connection() as conn:
            await conn.execute("UPDATE meetings SET deleted_at = ? WHERE id = ?", (now, meeting_id))
            await conn.commit()
    ...
    async def update_process(self, process_id: str, **kwargs):
        ...
        now = datetime.utcnow().isoformat()
        updates = {key: json.dumps(value) if isinstance(value, dict) else value for key, value in kwargs.items()}
    ...
    async def cleanup_old_processes(self, hours: int = 24) -> None:
        ...
        async with self._get_connection() as conn:
            await conn.execute("DELETE FROM summary_processes WHERE created_at < ?", (cutoff,))
            await conn.commit()
