...
$AppProjectName = "pulsetalq-backend"
$AppContainerName = "pulsetalq-backend"
$DefaultPort = 8178
$DefaultAppPort = 5167
...
function Invoke-StartCommand {
    ...
    $composeArgs += @("whisper-server", "pulsetalq-backend")
    ...
    if ($DryRun) {
        ...
        if ($detach) {
            ...
            & docker-compose -f $ComposeFile --profile default up -d whisper-server pulsetalq-backend
            ...
            & docker-compose -f $ComposeFile --profile default up whisper-server pulsetalq-backend
...
function Invoke-LogsCommand {
    ...
    $serviceMap = @{
        ...
        "app" = "pulsetalq-backend"
        "backend" = "pulsetalq-backend"
        "meeting" = "pulsetalq-backend"
    }
    ...
    if ($service) {
        ...
        $validServices = @("whisper-server", "pulsetalq-backend")
        if ($service -notin $validServices) {
            Write-Warn "Unknown service: $service"
            Write-Info "Available services: whisper-server, pulsetalq-backend"
        ...
        $logArgs += @("whisper-server", "pulsetalq-backend")
    }
