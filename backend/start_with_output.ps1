...
Write-Host "====================================="
Write-Host "PulseTalq Backend Startup"
Write-Host "====================================="
...
if (-not (Test-Path "whisper-server-package\whisper-server.exe")) {
    ...
    try {
        ...
        $apiUrl = "https://api.github.com/repos/Qblaauw/PulseTalq/releases/latest"
        ...
        $downloadUrl = "https://github.com/Qblaauw/PulseTalq/releases/download/$tagName/whisper-server.exe"
        ...
        try {
            $allReleasesUrl = "https://api.github.com/repos/Qblaauw/PulseTalq/releases"
            $headers = @{"User-Agent" = "PowerShell-Script"}
            ...
            if ($releases.Count -gt 0) {
                ...
                $altDownloadUrl = "https://github.com/Qblaauw/PulseTalq/releases/download/$latestTag/whisper-server.exe"
            ...
            Write-Host "https://github.com/Qblaauw/PulseTalq/releases"
...
Write-Host "====================================="
Write-Host "Starting PulseTalq Backend"
Write-Host "====================================="
...
# Check if pulsetalq-frontend is installed
$frontendInstalled = $false
$frontendPath = $null

# Check common installation paths for pulsetalq-frontend
$possiblePaths = @(
    "$env:LOCALAPPDATA\Programs\pulsetalq-frontend\pulsetalq-frontend.exe",
    "$env:LOCALAPPDATA\Programs\pulsetalq\pulsetalq-frontend.exe",
    "$env:ProgramFiles\pulsetalq-frontend\pulsetalq-frontend.exe",
    "${env:ProgramFiles(x86)}\pulsetalq-frontend\pulsetalq-frontend.exe",
    "$env:APPDATA\pulsetalq-frontend\pulsetalq-frontend.exe"
)
...
try {
    ...
    if ($regPath) {
        ...
        if (-not $frontendPath -and $regPath.InstallLocation) {
            ...
            $possibleExeNames = @("pulsetalq-frontend.exe", "pulsetalq.exe")
...
if ($frontendInstalled) {
    Write-Host "PulseTalq frontend application is installed."
    if ($frontendPath) {
        Write-Host "Location: $frontendPath"
        
        # Ask if user wants to launch the frontend
        $launchFrontend = Read-Host "Do you want to launch the PulseTalq frontend application? (Y/N)"
        if ($launchFrontend -eq 'Y' -or $launchFrontend -eq 'y') {
            Write-Host "Launching PulseTalq frontend..."
            Start-Process -FilePath $frontendPath
            Write-Host "PulseTalq frontend launched successfully."
        }
    }
} else {
    Write-Host "PulseTalq frontend application is not installed."
    Write-Host ""
    $installFrontend = Read-Host "Would you like to download and install the PulseTalq frontend application? (Y/N)"
        ...
        try {
            ...
            if ($setupAsset) {
                ...
                if ($installerProcess.ExitCode -eq 0) {
                    ...
                    # Check if pulsetalq is now installed and launch it
